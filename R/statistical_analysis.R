    ...
    result <- data %>%
      ...
      summarise(
        across(where(is.numeric), list(
          ...
          skewness = ~{library(moments); skewness(.x, na.rm = TRUE)},
          kurtosis = ~{library(moments); kurtosis(.x, na.rm = TRUE)}
        )),
        .groups = "drop"
      )
  } else {
...
perform_correlation_analysis <- function(data, method = "pearson", p_adjust = "holm") {
  ...
  # Calculate p-values
  cor_test_results <- cor.test_matrix(numeric_data, method = method)
  p_values <- cor_test_results$p.value
  
  # Adjust p-values
  p_adj <- matrix(p.adjust(p_values, method = p_adjust), 
                  nrow = nrow(p_values),
...
cor.test_matrix <- function(data, method = "pearson") {
  n <- ncol(data)
...
#' @export
perform_t_test <- function(data, value_var, group_var = NULL, mu = 0, 
  ...
  if (is.null(group_var)) {
    ...
    if (length(groups) != 2) {
      stop("Group variable must have exactly 2 levels for t-test")
    }
...
#' 
#' Conducts one-way ANOVA with post-hoc tests.
#' 
#' @param data A data frame
...
perform_anova <- function(data, value_var, group_var, post_hoc = TRUE) {
  ...
  if (post_hoc) {
    ...
    result$tukey <- tidy(tukey_result)
  }
...
#' 
#' Fits linear regression model with diagnostic plots.
#' 
#' @param data A data frame
...
perform_linear_regression <- function(data, formula, diagnostics = TRUE) {
  ...
  if (diagnostics) {
    result$diagnostics <- list(
      ...
      durbin_watson = car::durbinWatsonTest(model),
      vif = if(length(coefficients(model)) > 2) car::vif(model) else NULL
    )
  }
...
#' @return A data frame with growth rates
#' @export
calculate_growth_rate <- function(data, value_col, time_col, periods = 1) {
  ...
  # Calculate growth rate
  data$growth_rate <- c(rep(NA, periods), 
                       diff(data[[value_col]], lag = periods) / 
                       data[[value_col]][-((nrow(data)-periods+1):nrow(data))])
  
  data$growth_rate_pct <- data$growth_rate * 100
...
#' 
#' Basic time series analysis with trend and seasonality detection.
#' 
...
perform_time_series_analysis <- function(data, frequency = 12, forecast_periods = 6) {
  ...
  # Forecasting
  forecast_result <- forecast(auto.arima(ts_data), h = forecast_periods)
